...
class EventPushActionsWorkerStore(ReceiptsWorkerStore, StreamWorkerStore, SQLBaseStore):
    ...
    def _get_unread_counts_by_pos_txn(
        ...
        rows = txn.fetchall()
        for notif_count, unread_count, thread_id, _ in rows:
        ...
        rows = txn.fetchall()
        for highlight_count, thread_id, _ in rows:
            if not thread_id:
    ...
    def _get_notif_unread_count_for_user_room(
        ...
        return [row[:3] for row in txn.fetchall()]
    ...
    def _handle_new_receipts_for_notifs_txn(self, txn: LoggingTransaction) -> bool:
        ...
        for (room_id, user_id), rows in rows_by_room_user.items():
            for _, thread_id, stream_ordering in rows:
                txn.execute(
                    """
            ...
            earliest_stream_ordering = min(r[2] for r in rows)
            unread_counts = self._get_notif_unread_count_for_user_room(
                txn,
                room_id,
                user_id,
